...
function renderMembersDropdowns(containerId) {
...
const allStatuses = Object.values(paymentStatuses);
    ...
    member.boxes.forEach(boxId => {
      ...
      const statusClass = status === 'completed' ? 'completed' : status === 'delayed' ? 'delayed' : 'unpaid';
...
function cleanAndFormatPhone(phone) {
  ...
  // 3. إذا كان الرقم يبدأ بعلامة (+) (رغم أننا أزلناها، هذا للتأكيد) أو رمز دولة، نتركه كما هو.
  // إذا لم تبدأ بـ 0، نستخدمها كما هي (افتراضاً أنها دولية أو صالحة)
  return cleaned;
}

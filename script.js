...
function deleteTeam(teamId) {
    const message = 'هل أنت متأكد من حذف هذا الفريق؟ سيتم إزالته من جميع الأعضاء المشتركين فيه.';
    showConfirmationModal(message, () => {
        const teams = getFromStorage('teams', []);
        const filteredTeams = teams.filter(t => t.id !== teamId);
        saveToStorage('teams', filteredTeams);

        // إزالة الفريق من الأعضاء
        const members = getAllMembers();
        members.forEach(member => {
            if (member.teams && member.teams.includes(teamId)) {
                member.teams = member.teams.filter(id => id !== teamId);
            }
        });
        saveToStorage('members', members);

        showAlert('تم حذف الفريق بنجاح', 'success');
        renderTeamsTable('teams-table-container'); // إعادة عرض الجدول
    });
}
...
function deleteBox(boxId) {
    const message = 'هل أنت متأكد من حذف هذا الصندوق؟ سيتم حذفه من جميع الأعضاء المشتركين فيه.';
    showConfirmationModal(message, () => {
        const boxes = getFromStorage('boxes', []);
        const filteredBoxes = boxes.filter(b => b.id !== boxId);
        saveToStorage('boxes', filteredBoxes);

        // إزالة الصندوق من الأعضاء
        const members = getAllMembers();
        members.forEach(member => {
            if (member.boxes && member.boxes.includes(boxId)) {
                member.boxes = member.boxes.filter(id => id !== boxId);
            }
        });
        saveToStorage('members', members);

        showAlert('تم حذف الصندوق بنجاح', 'success');
        renderBoxesTable('boxes-table-container'); // إعادة عرض الجدول
    });
}
...
function deleteMember(memberId) {
    const message = 'هل أنت متأكد من حذف هذا العضو؟ لا يمكن التراجع عن هذا الإجراء.';
    showConfirmationModal(message, () => {
        const members = getFromStorage('members', []);
        const filtered = members.filter(m => m.id !== memberId);
        saveToStorage('members', filtered);
        showAlert('تم حذف العضو بنجاح', 'success');
        renderMembersDropdowns('members-dropdowns-container'); // إعادة عرض القائمة
    });
}
...
function editMember(memberId) {
    const message = 'هل أنت متأكد من رغبتك في تحرير بيانات هذا العضو؟';
    showConfirmationModal(message, () => {
        // في المستقبل، يمكن استدعاء دالة فتح نافذة التحرير من هنا
...
/**
 * عرض رسالة تنبيه (Toast Notification)
 * @param {string} message - نص الرسالة
...
function showAlert(message, type = 'info') {
    ...
    if (!container) {
        ...
        document.documentElement.appendChild(container); // ⭐️ تم التعديل: إضافة الحاوية إلى <html> بدلاً من <body>
    }
...
/**
 * ==========================================================
 *  التحكم في نافذة التأكيد المنبثقة (Confirmation Modal)
 * ==========================================================
 */
...
/**
 * يعرض نافذة التأكيد برسالة مخصصة.
 * @param {string} message - الرسالة التي ستظهر للمستخدم.
 * @param {function} callback - الدالة التي ستُنفذ عند الضغط على "تأكيد".
 */
function showConfirmationModal(message, callback) {
    const confirmationModal = document.getElementById('confirmationModal');
    const confirmationModalText = document.getElementById('confirmationModalText');

    if (!confirmationModal || !confirmationModalText) {
        console.error('عناصر نافذة التأكيد غير موجودة في الصفحة.');
        return;
    }
...
function closeConfirmationModal() {
    ...
    onConfirmCallback = null; // إعادة تعيين الدالة
}

function handleConfirm() {
    if (typeof onConfirmCallback === 'function') {
        onConfirmCallback(); // تنفيذ الدالة المخزنة
    }
        ...
        html += `
            <div class="card-item team-card">
                ...
                <div class="card-item-actions">
                    <button class="btn sm icon-frameless" onclick="openEditTeamModal(${team.id})" title="تحرير"><i class="fas fa-edit" style="color: var(--text-secondary);"></i></button>
                    <button class="btn sm icon-frameless" onclick="deleteTeam(${team.id})" title="حذف"><i class="fas fa-trash-alt" style="color: var(--error-color);"></i></button>
                </div>
            </div>
        `;
    });
    ...
    boxes.forEach(box => {
        ...
        html += `
            <div class="card-item">
                ...
                <div class="card-item-actions">
                    <button class="btn sm icon-frameless" onclick="openEditBoxModal(${box.id})" title="تحرير"><i class="fas fa-edit" style="color: var(--text-secondary);"></i></button>
                    <button class="btn sm icon-frameless" onclick="deleteBox(${box.id})" title="حذف"><i class="fas fa-trash-alt" style="color: var(--error-color);"></i></button>
                </div>
            </div>
        `;
    });
            ...
            <div class="member-dropdown">
                ...
                <div class="member-dropdown-body">
                    ...
                    <div style="border-top: 1px solid var(--border-color); padding-top: 15px; margin-top: 15px; display: flex; gap: 10px;">
                        <button class="btn sm icon-frameless" onclick="editMember(${member.id})" title="تحرير"><i class="fas fa-edit" style="color: var(--text-secondary);"></i></button>
                        <button class="btn sm icon-frameless" onclick="deleteMember(${member.id})" title="حذف"><i class="fas fa-trash-alt" style="color: var(--error-color);"></i></button>
                    </div>
                </div>
            </div>
        `;
...
document.addEventListener('click', function (event) {
    if (event.target.classList.contains('modal-close')) {
        ...
        const modal = event.target.closest('.modal');
        if (modal && modal.id === 'confirmationModal') {
            modal.classList.remove('show');
        }
    }
});
